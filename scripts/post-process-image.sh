...
DATE=$(date +%Y%m%d)
IMAGE_NAME="umbrel-pipa-${DATE}"
RAW_IMAGE="${1:-$REPO_ROOT/work/umbrel-pipa.img}"
...
truncate -s "${SIZE}M" "$OUTPUT_DIR/umbrel_rootfs.raw"
MKE2FS_DEVICE_PHYS_SECTSIZE=4096 MKE2FS_DEVICE_SECTSIZE=4096 \
    mkfs.ext4 -L "$ROOTFS_LABEL" "$OUTPUT_DIR/umbrel_rootfs.raw"
ROOT_MNT=$(mktemp -d)
mount -o loop "$OUTPUT_DIR/umbrel_rootfs.raw" "$ROOT_MNT"
...
truncate -s 1024M "$OUTPUT_DIR/umbrel_boot.raw"
mkfs.ext4 -F -L "$BOOT_LABEL" -O ^64bit,^metadata_csum,^metadata_csum_seed,^orphan_file "$OUTPUT_DIR/umbrel_boot.raw"
mount -o loop "$OUTPUT_DIR/umbrel_boot.raw" "$BOOT_MNT"
...
truncate -s 128M "$OUTPUT_DIR/umbrel_esp.raw"
mkfs.fat -F 16 -n "$ESP_LABEL" "$OUTPUT_DIR/umbrel_esp.raw"
mount -o loop "$OUTPUT_DIR/umbrel_esp.raw" "$ESP_MNT"
...
announce "This mode flashes umbrelOS rootfs to userdata."
...
echo "umbrelOS EFI image      -> rawdump"
echo "umbrelOS boot           -> cust"
echo "umbrelOS rootfs         -> userdata"
...
announce "Flashing umbrelOS EFI image to rawdump"
fastboot flash rawdump umbrel_esp.raw

announce "Flashing umbrelOS boot image to cust"
fastboot flash cust umbrel_boot.raw

announce "Flashing umbrelOS rootfs image to userdata"
fastboot flash userdata umbrel_rootfs.raw
...
announce "This mode flashes umbrelOS rootfs to a dedicated partition such as linux."
...
echo "Flash vbmeta_ab       -> $FLASH_VBMETA"
echo "umbrelOS EFI image      -> $ESP_PARTITION"
echo "umbrelOS boot           -> $BOOT_PARTITION"
echo "umbrelOS rootfs         -> $ROOTFS_PARTITION"
echo "Erase dtbo_ab         -> $ERASE_DTBO"
echo "Refresh A/B slot      -> yes (set_active toggle)"
echo
echo
...
announce "Flashing umbrelOS EFI image to $ESP_PARTITION"
fastboot flash "$ESP_PARTITION" umbrel_esp.raw

announce "Flashing umbrelOS boot image to $BOOT_PARTITION"
fastboot flash "$BOOT_PARTITION" umbrel_boot.raw

announce "Flashing umbrelOS rootfs image to $ROOTFS_PARTITION"
fastboot flash "$ROOTFS_PARTITION" umbrel_rootfs.raw
